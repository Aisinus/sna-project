...
from app.form import UserLoginForm, UserCreateForm
from app.model import connect_db, User, AuthToken
...
def create_user(user: UserCreateForm = Body(..., embed=True), database=Depends(connect_db)):
    ...
    if exist_user:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Email already exitst')
